"""
Train catalyst-aware model WITH catalyst detection.

This includes catalyst_type (earnings, gap, unknown) as a feature.
Should beat baseline if catalyst detection helps.

Features: move size, volume, drift, VIX, sector, + CATALYST_TYPE
...
def prepare_features_catalyst_aware(df):
    """
    Prepare features for CATALYST-AWARE model.

    Includes catalyst_type as a feature.
    ...
    # One-hot encode categorical features (including catalyst_type!)
    data = pd.get_dummies(data, columns=['sector', 'market_cap_bucket', 'catalyst_type'], drop_first=True)

    # Select features (INCLUDING catalyst_type!)
    feature_cols = [
        'day0_return_abs',
        'direction',
        'volume_ratio',
...
def train_catalyst_aware_model():
    """Train XGBoost model WITH catalyst detection."""

    print("\n" + "=" * 60)
    print("CATALYST-AWARE MODEL (WITH CATALYST DETECTION)")
    print("=" * 60 + "\n")
    ...
    # Show catalyst distribution
    print("      Catalyst distribution:")
    catalyst_counts = df['catalyst_type'].value_counts()
    for catalyst, count in catalyst_counts.items():
        print(f"        {catalyst}: {count} ({count/len(df)*100:.1f}%)")
    print()

    # Prepare features
    print("[2/5] Preparing features (INCLUDING catalyst_type)...")
    X, y, feature_names = prepare_features_catalyst_aware(df)
    print(f"      Features: {len(feature_names)}")
    catalyst_features = [f for f in feature_names if 'catalyst_type' in f]
    print(f"      Catalyst features: {catalyst_features}")
    ...
    for i, row in feature_importance.head(15).iterrows():
        highlight = " <-- CATALYST" if 'catalyst_type' in row['feature'] else ""
        print(f"  {row['feature']:30s} {row['importance']:.4f}{highlight}")

    print("\n" + "=" * 60)
    print(f"CATALYST-AWARE ACCURACY: {test_acc:.1%}")
    print("=" * 60 + "\n")
...
if __name__ == "__main__":
    results = train_catalyst_aware_model()
